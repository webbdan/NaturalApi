...
/// <summary>
/// RestSharp-specific implementation of IApiResultContext.
/// Wraps RestSharp's RestResponse for NaturalApi compatibility.
/// </summary>
public class RestSharpApiResultContext : IApiResultContext
{
    ...
    /// </summary>
    public IDictionary<string, string> Headers => _response.Headers?.ToDictionary(h => h.Name, h => h.Value?.ToString() ?? "") ?? new Dictionary<string, string>();
    ...
    /// <summary>
    /// Gets the RestSharp response.
    /// </summary>
    public RestResponse RestResponse => _response;
    ...
    public IApiResultContext Then(Action<IApiResult> next)
    {
        ...
        var result = new RestSharpApiResult(this, _executor);
        next(result);
        return this;
    }
    ...
    public string? GetCookie(string name)
    {
        ...
        var setCookieHeaders = _response.Headers?.Where(h => h.Name.Equals("Set-Cookie", StringComparison.OrdinalIgnoreCase));
        if (setCookieHeaders != null)
        {
            foreach (var setCookieHeader in setCookieHeaders)
            {
                var cookieValue = ExtractCookieValue(setCookieHeader.Value?.ToString(), name);
                if (cookieValue != null)
                    return cookieValue;
            }
        }
    ...
    /// <summary>
    /// Creates an HttpResponseMessage from the RestSharp response for compatibility.
    /// </summary>
    /// <returns>HttpResponseMessage representation</returns>
    private HttpResponseMessage CreateHttpResponseMessage()
    {
        var response = new HttpResponseMessage((System.Net.HttpStatusCode)_response.StatusCode);
        ...
        {
            response.Content = new StringContent(_response.Content, System.Text.Encoding.UTF8, "application/json");
        }
...
/// <summary>
/// RestSharp-specific implementation of IApiResult.
/// </summary>
public class RestSharpApiResult : IApiResult
{
    private readonly RestSharpApiResultContext _context;
    private readonly RestSharpHttpExecutor _executor;

    public RestSharpApiResult(RestSharpApiResultContext context, RestSharpHttpExecutor executor)
    {
        _context = context;
        _executor = executor;
    }
